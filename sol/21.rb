...
foods = DATA.each_line.map do |line|
  food = line.scan /\w+/
...
puts ingredient_could_be.sum { |ingredient, allergens| allergens.empty? ? occurrence_counter[ingredient] : 0 } # 2493
...
puts matches.sort_by(&:last).map(&:first).join(?,) # kqv,jxx,zzt,dklgl,pmvfzk,tsnkknk,qdlpbt,tlgrhdh
